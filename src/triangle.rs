...
fn main() -> AppResult<()> {
    ...
    //only recreate renderpass im neccessary
                ...
                let res: AppResult<()> = try {
                    unsafe {
                        ...
                        if window_extent != extent {
                            Err(Error::VkResult(VkResult::SUBOPTIMAL_KHR))?
                        }

                        device.wait_for_fences(&[in_flight], true, u64::MAX)?;

                        let image_index = swapchain.acquire_next_image(swapchain_khr, u64::MAX, image_available, Fence::null())?;
                        if image_index.1 {
                            Err(Error::VkResult(VkResult::SUBOPTIMAL_KHR))?
                        }
                        ...
                        if swapchain.queue_present(queue, &present_info)? {
                            Err(Error::VkResult(VkResult::SUBOPTIMAL_KHR))?
                        }
                    }
                };
                match res {
                    Err(Error::VkResult(VkResult::ERROR_OUT_OF_DATE_KHR)) | Err(Error::VkResult(VkResult::SUBOPTIMAL_KHR)) => {
                        let res: AppResult<()> = try {
                            unsafe { 
...
fn create_device(instance: &Instance, physical_device: PhysicalDevice, queue_family_index: u32) -> AppResult<Device> {
    ...
    let create_info = DeviceCreateInfo {
        ...
        pp_enabled_extension_names: [b"VK_KHR_swapchain\0".as_ptr() as *const i8].as_ptr(),
...
fn create_debug_utils_messenger(debug_utils: &DebugUtils) -> AppResult<DebugUtilsMessengerEXT> {
    ...
    extern "system" fn debug_callback(severity: DebugUtilsMessageSeverityFlagsEXT,
...
pub fn create_instance(entry: &Entry, window: &Window) -> AppResult<Instance> {
    ...
    let extensions = [&[b"VK_EXT_debug_utils\0".as_ptr() as *const i8],
        ash_window::enumerate_required_extensions(window)?].concat();
    ...
    let create_info = InstanceCreateInfo {
        ...
        pp_enabled_layer_names: [b"VK_LAYER_KHRONOS_validation\0".as_ptr() as *const i8].as_ptr()
    };
